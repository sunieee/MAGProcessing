...
def relative_positions(list_of_citations:list,paper_json:dict):  #返回多次的最大值最小值均值
    ...
    for index in list_of_citations:
        ...
        if section_id == -1:
            relative_position_sentence.append(-2)
            relative_position_subsection.append(-2)
            relative_position_section.append(-2)
        else:
    ...
    while len(relative_position_sentence) > 1:
        if ii == len(relative_position_sentence):
            ...
            if relative_position_sentence[ii] == -2:
                relative_position_sentence.pop(ii)
                relative_position_subsection.pop(ii)
                relative_position_section.pop(ii)
            else:
                ii += 1
    result_1 = [min(relative_position_section),max(relative_position_section)-min(relative_position_section)]
    result_2 = [min(relative_position_subsection),max(relative_position_subsection)-min(relative_position_subsection)]
    result_3 = [min(relative_position_sentence),max(relative_position_sentence)-min(relative_position_sentence)]
...
def content_key_phrases(list_of_citations:list,paper_json:dict):   #关键句式
    a = ["adapt", "adjust", "augment", "combine", "change", "decrease", "elaborate"]
    b = ["expand", "expand on", "extend", "derive", "incorporate", "increase"]
    c = ["manipulate", "modify", "optimize", "optimise", "refine", "render"]
    d = ['our previous',"Our baseline", "our earlier", "from our", "we employ",'based model']
    e = ['As our', 'We follow', 'our previous',"Our baseline", "referred to"]
    f = ["perform better", "outperform", "outweigh", "surpass"]
    g = ["following our", "follows the", "build on",'builds on','follow the']
    h = ["our method", "Our baseline", "we made", 'start from', 'proceed from']
    key_phrases = [d,
                   ['extension'],
                   ['We follow'],
                   ['previous'],
    ...
    for index, citation in enumerate(paper_json['citation_contexts']):
        if index in list_of_citations and citation['cite_context']!= -1:
            ...
            for i,keys in enumerate(key_phrases):
                for key in keys:
                    result[i] += len(re.findall(key, sentences,re.IGNORECASE))
...
# file = open(r'C:\Users\alienware\Documents\WeChat Files\wxid_fqia26wxom2v21\FileStorage\File\2022-11\all_features_acl_hInewAuthors_links.csv','r')
ID_transform_file = open('../MAG_ID_TO_EXTERNAL_AND_TITLE.csv','r')
new_data_file = open('../all_dataset_link_label_with_combined_features.txt','w')
# new_data_file = open('../data_needed.txt','w')
new_sentence_file = open('../sentences_and_labels.csv','w',encoding='utf-8')
feature_name = []
line = file.readline()
list_names = ['average_section_citation','average_subsection_citation','average_sentence_citation',
...
#first_line = line.strip()+','.join(['']+list_names)+'\n'
#new_data_file.writelines(first_line)
...
while len(line) > 3:
    ...
    if list_of_citations == []:
        ...
        for ids in list_of_citations:
            ...
            label = line[2]
            # new_sentence_file.writelines('\t'.join([line[0],line[1],label,sentence])+'\n')
    ...
    # chosen = [0,1,2,5,8,9,10,12,13,16,19]+[i for i in range(20,len(line))]
    # line = [str(line[x]) for x in chosen]
    line = [str(x) for x in line]
    # line.insert(3,str(len(list_of_citations)))
