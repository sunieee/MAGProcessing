...
create table firstAuthorTmp 
    ...
    from authors_field as A 
    join paper_author_field as PA1 on A.authorID = PA1.authorID 
    join paper_author_field as PA2 on PA1.paperID = PA2.paperID 
...
select firstAuthorID, topAuthorID, PA1.authorOrder as firstAuthorOrder, year, count(*) as cnt 
    from firstAuthorTmp 
    join paper_author_field as PA1 on firstAuthorID = PA1.authorID 
    join paper_author_field as PA2 on topAuthorID = PA2.authorID 
        and PA1.paperID = PA2.paperID and PA1.authorOrder <= {MIN_STUDENT_AUTHOR_ORDER} 
        and PA1.authorOrder < PA2.authorOrder 
    join papers_field as P on PA1.paperID = P.paperID 
...
######################################################################
print("compute year count maps!", datetime.datetime.now().strftime("%H:%M:%S"))
...
select A.authorID, year, count(*) as cnt 
    ...
    join papers_field as P on PA.paperID = P.paperID 
