...
init_tmpsql="set @row_number=0;"
createAuthorsFieldhI = "create table scigene_$$fieldNAME$$_field.authors_field_hI select authorID, `rank`, name, PaperCount, CitationCount, PaperCount_field, ( @row_number := @row_number + 1 ) AS authorRank, CitationCount_field, hIndex_field, FellowType from scigene_$$fieldNAME$$_field.authors_field order by hIndex_field desc;"
dropAuthorsFieldhI = "drop table scigene_$$fieldNAME$$_field.authors_field_hI"
createAuthorsFieldhI_Index = "create index %s_index on scigene_$$fieldNAME$$_field.authors_field_hI(%s)"
#change 1 row, and it is already back
# selectTopfieldAuthors = "select authorID, name, authorRank, PaperCount_field from scigene_$$fieldNAME$$_field.authors_field_hI where authorRank > 200 and authorRank <= $$NUMTOPAUTHORS$$;"
selectTopfieldAuthors = "select authorID, name, authorRank, PaperCount_field from scigene_$$fieldNAME$$_field.authors_field_hI where authorRank <= $$NUMTOPAUTHORS$$;"
...
updatefieldPaperFirstAuthorName = "update papers_$$fieldAUTHOR$$ as P, scigene_$$fieldNAME$$_field.authors_field_hI as A set P.firstAuthorName = A.name where P.firstAuthorID = A.authorID"
...
dropAuthorsFieldhI = dropAuthorsFieldhI.replace(field_NAME, fieldName)
createAuthorsFieldhI = createAuthorsFieldhI.replace(field_NAME, fieldName)
createAuthorsFieldhI_Index = createAuthorsFieldhI_Index.replace(field_NAME, fieldName)
...
# creare authors_field_hI
try:
    db_cursor.execute(dropAuthorsFieldhI)
    db_cursor.commit()
    print('authors_field_hI exist! try to drop table and rebuild...')
except:
    print('authors_field_hI not found! try to creat table...')
db_cursor.execute(init_tmpsql)
db_cursor.execute(createAuthorsFieldhI)
db_cursor.commit()
for col in ['authorID','authorRank','name']:
    db_cursor.execute(createAuthorsFieldhI_Index%(col,col))
    db_cursor.commit()
...
for row in rows:
    ...
    authorTableName = "".join(filter(str.isalpha, authorName)).lower() + str(rank)  # xiaofengli1
