...
# 方法2，提取top-1000学者的论文
    ...
    for i in range(1, 1001):
        df = pd.read_csv("./input/vis/papers_" + str(i) + ".csv", sep=',', index_col=0)
        df = df.fillna('')
...
#     fig.write_html("./vis_output/topics_visualization.html")
#     sys.exit()
...
topic_model.save("./model/model-visualization-mpnet")   # TODO 模型name

# 打印TopicID对应的Count和Name（去掉TopicID=-1，因为它都是一些to the and之类的topic）
df_topics = topic_model.get_topic_info()
df_topics = df_topics.loc[df_topics.Topic != -1, :]
df_topics.to_csv("./output/vis-mpnet-120/topic-count-name.csv", sep=',', index=False)
...
with open("./output/vis-mpnet-120/topic-word-prob.json", "w") as f:
    f.write(data)
...
# 可视化topic分布2d图
fig = topic_model.visualize_topics()
fig.write_html("./output/vis-mpnet-120/topic-visualization.html")
...
for i in range(1, 1001):
    ...
    df["topic"] = topicidx
    df.to_csv("./csv/papers_" + str(i) + ".csv", sep=',')
